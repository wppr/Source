#ifndef SCENE_NODE_EXT_H
#define SCENE_NODE_EXT_H
#include "Mathlib.h"
#include "Movable.h"
...
namespace HW
{
	...
	*/
	...
	//=========================== common ============================================
		...
		// local space
		inline void scale(Vector3& scal)
		{
			m_scale *= scal;
		...
		{
			// get rotation matrix
		//	Matrix4 world(getOrientation());
		//	world.setTrans(getTranslation());
			//Matrix4 world ;
			if(WorldMatrixDirty)
				WorldMatrix.makeTransform(getTranslation(),getScale(),getOrientation());
			return WorldMatrix;
		}
		...
		const Vector3& getBoundingCenter();
		...
		// make sure there is at least one entity.
		const Vector3& getGlobalBoundingCenter();
