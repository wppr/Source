...
void readMaterial(aiMaterial* mat,as_Material* my_mat){
	...
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_DIFFUSE, 0, &path)){
		string tex_type = "diffuse";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_SPECULAR, 0, &path)){
		string tex_type = "specular";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_AMBIENT, 0, &path)){
		string tex_type = "ambient";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}

	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_OPACITY, 0, &path)){
		string tex_type = "opacity";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}

	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_HEIGHT, 0, &path)){
		string tex_type = "height";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_EMISSIVE, 0, &path)){
		string tex_type = "emissive";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_NORMALS, 0, &path)){
		string tex_type = "normal";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_SHININESS, 0, &path)){
		string tex_type = "shininess";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_DISPLACEMENT, 0, &path)){
		string tex_type = "displacement";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_REFLECTION, 0, &path)){
		string tex_type = "reflection";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
	if (aiReturn_SUCCESS == mat->GetTexture(aiTextureType_LIGHTMAP, 0, &path)){
		string tex_type = "lightmap";
		my_mat->textures[tex_type] = new as_Texture(string(name.C_Str()) + tex_type);
		my_mat->textures[tex_type]->path = path.C_Str();
	}
