...
FRHIRenderTarget* FOpenGLDynamicRHI::RHICreateRenderTarget(const FRHISetRenderTargetsInfo& Info)
{
	FOpenGLRenderTarget* RT = new FOpenGLRenderTarget;
	RT->Info = Info;
	RT->BindTexture();
	FOpenGLContextState& ContextState = GetContextStateForCurrentContext();
	ContextState.Framebuffer = RT->Resource;
	if (RT->bValid)
		return RT;
	else
		return NULL;
...
void FOpenGLRenderTarget::BindTexture()
{
	if (Resource == 0)
	...
	for (int i = 0; i < Info.NumColorRenderTargets; i++)
	{
		auto Texture= GetOpenGLTextureFromRHITexture(Info.ColorRenderTarget[i].Texture);
		...
		{
			case GL_TEXTURE_2D:
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, Texture->Resource, 0);
				break;
			case GL_TEXTURE_CUBE_MAP:
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_CUBE_MAP_POSITIVE_X+ Info.ColorRenderTarget[i].ArraySliceIndex, Texture->Resource, 0);
				break;
		default:
			break;
		}
	}
	auto DepthTexture = GetOpenGLTextureFromRHITexture(Info.DepthStencilRenderTarget.Texture);
	if (DepthTexture!= NULL)
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, DepthTexture->Resource, 0);
...
void FOpenGLDynamicRHI::RHISetRenderTarget(FRHIRenderTarget* RenderTarget)
{
	auto RT = static_cast<FOpenGLRenderTarget*>(RenderTarget);
	if (RT)
		PendingState.Framebuffer = RT->Resource;
}