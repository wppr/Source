...
//
//
FRHITexture* FOpenGLDynamicRHI::CreateOpenGLTexture(uint32 SizeX, uint32 SizeY, bool bCubeTexture, bool bArrayTexture, uint8 Format, uint32 NumMips, uint32 NumSamples, uint32 ArraySize, uint32 Flags, const FClearValueBinding& InClearValue, vector<void*> BulkData)
{
	...
	{
		Target = bArrayTexture ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
	}
...
#define Max(a,b) ((a)>(b)?(a):(b))
					for(uint32 TargetIndex = 0; TargetIndex < NumTargets; TargetIndex++)
					{
						uint8* Data = (uint8*)BulkData[TargetIndex];
						glTexSubImage2D(
							/*Target=*/ FirstTarget + TargetIndex,
							/*Level=*/ MipIndex,
							/*XOffset*/ 0,
							/*YOffset*/ 0,
							/*SizeX=*/ Max(1,(SizeX >> MipIndex)),
							/*SizeY=*/ Max(1,(SizeY >> MipIndex)),
							/*Format=*/ GLFormat.Format,
							/*Type=*/ GLFormat.Type,
							/*Data=*/ &Data[MipOffset]
							);
				...
				//uint32 NumBlocksX =Max(1,(SizeX >> MipIndex) / GPixelFormats[Format].BlockSizeX);
				//uint32 NumBlocksY =Max(1,(SizeY >> MipIndex) / GPixelFormats[Format].BlockSizeY);
				//uint32 NumLayers = Max(1,ArraySize);
	...
	{
		//assert( FOpenGL::SupportsMultisampledTextures() );
		//assert( BulkData.size()==0);
	...
	//		assert(GMaxOpenGLColorSamples>=(GLint)NumSamples);
	//		break;
	//	case GL_DEPTH_ATTACHMENT:
	//	case GL_DEPTH_STENCIL_ATTACHMENT:
	//		assert(GMaxOpenGLDepthSamples>=(GLint)NumSamples);
	//		break;
	//	default:
	//		break;
	//}
...
{
	shared_ptr<FRHITexture2D> Texture((FRHITexture2D*)CreateOpenGLTexture(SizeX, SizeY, false, false, Format, NumMips, NumSamples, 1, Flags, Info.ClearValueBinding, Info.BulkData));
	return Texture;
}
//
...
FTextureCubeRHIRef FOpenGLDynamicRHI::RHICreateTextureCube( uint32 Size, uint8 Format, uint32 NumMips, uint32 Flags, FRHIResourceCreateInfo& CreateInfo )
{
	...
	//check(!CreateInfo.BulkData);
	 shared_ptr<FRHITextureCube> t((FRHITextureCube*)CreateOpenGLTexture(Size,Size,true, false, Format, NumMips, 1, 1, Flags, CreateInfo.ClearValueBinding,CreateInfo.BulkData));
	 return t;
}
//
