...
{
//	VERIFY_GL_SCOPE();
	FOpenGLBoundShaderState* State = ResourceCast(BoundShaderStateRHI);
	PendingState.BoundShaderState = State;
...
void FOpenGLDynamicRHI::RHISetBoundBufferState(FBoundBufferStateRHIParamRef BoundBufferState)
{
	FOpenGLBoundBufferState* State = ResourceCast(BoundBufferState);
	PendingState.BoundBufferState = State;

}

void FOpenGLDynamicRHI::BindPendingBufferState(FOpenGLContextState& ContextState)
{

	GLuint PendingVAO = PendingState.BoundBufferState->VAO;
	
	if (ContextState.VAO != PendingVAO)
	{
		glBindVertexArray(PendingVAO);
		ContextState.VAO = PendingVAO;
	}
}
...
void FOpenGLDynamicRHI::SetupTexturesForDraw( FOpenGLContextState& ContextState, const StateType ShaderState, int32 MaxTexturesNeeded )
{
	...
	for (int32 TextureStageIndex = 0; TextureStageIndex <= MaxTexturesNeeded; ++TextureStageIndex)
	{
		const FTextureStage& TextureStage = PendingState.Textures[TextureStageIndex];
		if(TextureStage.Resource==0)
...
void FOpenGLDynamicRHI::RHISetShaderSampler(FVertexShaderRHIParamRef VertexShader, uint32 SamplerIndex, FSamplerStateRHIParamRef NewStateRHI)
{
...
void FOpenGLDynamicRHI::RHISetShaderSampler(FPixelShaderRHIParamRef PixelShader, uint32 SamplerIndex, FSamplerStateRHIParamRef NewStateRHI)
{
	FOpenGLSamplerState* NewState = ResourceCast(NewStateRHI);
	if (FOpenGL::SupportsSamplerObjects())
	{
...
void FOpenGLDynamicRHI::RHIDrawPrimitive(uint32 PrimitiveType,uint32 BaseVertexIndex,uint32 NumPrimitives,uint32 NumInstances)
{
	...
	BindPendingBufferState(ContextState);
