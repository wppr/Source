...
//			check(Type == GL_ARRAY_BUFFER || Type == GL_ELEMENT_ARRAY_BUFFER);
			...
			//uint8* Data = (uint8*) glMapBufferRange(Type, GL_WRITE_ONLY);
	...
	//{
	//	glFramebufferTexture2DEXT(Target, Attachment, Texture, Level);
	//}
	...
	static FORCEINLINE void DepthRange(GLfloat Near, GLfloat Far)
	{
...
#define Max(a,b) ((a)>(b)?(a):(b))
		...
		{
			Params[Idx] = Max( float(Values[Idx]) / float(Max), -1.0f);
		}
	...
	static FORCEINLINE void ClearDepth(GLfloat Depth)
	{
		glClearDepthf(Depth);
	}
