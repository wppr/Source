...
void GLProgramDataSetter::UpdateGlobalVariable(const map<unsigned int, string> &uniformMap)
{
	...
	for(auto i = uniformMap.begin(); i != uniformMap.end(); i++)
	{
		...
		case GVS_POST_TEXTURE:
		{
			...
			SetProgramTexture(i->second, texture[0], program->texture_count);
		}break;
		default:;
		}
	}
	...
	updateProgramData(program->m_ProgramData);
}
...
void GLProgramDataSetter::updateProgramData(const map<string, ProgramData> & program_data)
{
